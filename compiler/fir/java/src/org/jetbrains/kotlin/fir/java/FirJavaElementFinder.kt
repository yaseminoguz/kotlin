...
class FirJavaElementFinder(
    ...
    override fun findClass(qualifiedName: String, scope: GlobalSearchScope): PsiClass? {
        ...
        val firClass =
            firProvider.getFirClassifierByFqName(classId)
                as? FirRegularClass
                ?: return null
...
private fun ConeClassType.mapToCanonicalString(session: FirSession): String {
    ...
    with(context) {
        ...
        typeConstructor.getPrimitiveArrayType()?.let { JvmPrimitiveType.get(it).javaKeywordName + "[]" }
        val kotlinClassFqName = typeConstructor.getClassFqNameUnsafe()?.toSafe() ?: return ERROR_TYPE_STUB
        val mapped = JavaToKotlinClassMap.mapJavaToKotlin(kotlinClassFqName) ?: kotlinClassFqName
...
private fun ConeKotlinTypeProjection.mapToCanonicalString(session: FirSession): String {
    return when (this) {
        ...
        is ConeTypedProjection -> {
            val keyword = if (kind == ProjectionKind.IN)
                "super"
            else
                "extends"

            "? $keyword ${(type as ConeClassLikeType).mapToCanonicalString(session)}"
        }
        else -> ERROR_TYPE_STUB
    } + "[]"
}
