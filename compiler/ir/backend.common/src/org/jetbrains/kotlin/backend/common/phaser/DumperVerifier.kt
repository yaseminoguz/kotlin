...
fun dumpIrElement(phaseState: PhaseState, data: IrElement, context: Any?): String {
    val beforeOrAfterStr = phaseState.beforeOrAfter.name.toLowerCase()

    var dumpText: String = ""
    val elementName: String

    val dumpOnlyFqName = phaseState.config.dumpOnlyFqName
    ...
    val title = "-- IR for $elementName $beforeOrAfterStr ${phaseState.phase.description}\n"
...
fun <Data, Context> dumpToFile(
    ...
    fun(phaseState: PhaseState, data: Data, context: Context) {
        val directoryPath = phaseState.config.dumpToDirectory ?: return
        val dumpContent = dumper(phaseState, data, context) ?: return
...
fun <Data, Context> dumpToStdout(
    ...
    fun(phaseState: PhaseState, data: Data, context: Context) {
        if (phaseState.config.dumpToDirectory != null) return
        val dumpContent = dumper(phaseState, data, context) ?: return
        println("\n\n----------------------------------------------")
        println(dumpContent)
        println()
