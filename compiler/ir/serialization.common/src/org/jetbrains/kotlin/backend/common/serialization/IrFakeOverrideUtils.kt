...
/**
 * TODO: This method can be simplified if the overriddenSymbols list of an IrFiled always includes only one element.
 * Currently it's not the case for some cinterop libraries (e.g. Foundation). In these libraries interfaces containing
 * a final property with an external getter are created (corresponding compiler error is suppressed). Due to KT-33081
 * such property gets a backing field and a field of a class implementing such interfaces can have several elements
