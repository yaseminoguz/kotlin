    ...
    private fun getOrRegisterDisabledTargets(project: Project) =
        project.rootProject.extensions.run {
            if (findByName(EXTENSION_NAME) == null) {
                ...
                printWarningWhenTaskGraphReady(project)
            }
            getByName(EXTENSION_NAME)
        } as MutableList<DisabledTarget>

    private fun printWarningWhenTaskGraphReady(project: Project) {
        project.gradle.taskGraph.whenReady {
            ...
            project.logger.warn(buildString {
                ...
                disabledTargetGroups.forEach { (targetProject, targetsBySupportedHosts) ->
                    ...
                    targetsBySupportedHosts.forEach { (supportedHosts, targets) ->
                        append("        * target")
                        append(targets.joinToString(prefix = "s".takeIf { targets.size > 1 }.orEmpty().plus(" ")) { "'${it.target.name}'" })
                        val supportedHostsString =
                            if (supportedHosts.size == 1)
                                "a ${supportedHosts.single()} host"
                            else
                                "one of the hosts: ${supportedHosts.joinToString(", ")}"
                        appendln(" (can be built with $supportedHostsString)")
                    }
                }
                appendln("\nTo disable this message, add '$DISABLE_WARNING_PROPERTY_NAME=true' to the Gradle properties.")
            })
        }
    }
}